...
const CategoryMenu = ({ className = '', variant = 'nav' }) => {
    ...
    return (
        <div className='relative'>
            <div className={className || (variant === 'nav' ? 'flex flex-wrap justify-baseline gap-4 bg-gray-200 py-2 px-8 font-light' : 'flex flex-wrap max-w-[40vw] h-30 gap-2')}>
                <NavLink to={`/product`} className='cursor-pointer hover:text-lime-400 transition-all duration-300'>Shop ALL</NavLink>
                {categories.map((ct) => (
                    <div key={ct.name} className='text-center cursor-pointer hover:text-lime-400 transition-all duration-300'>
