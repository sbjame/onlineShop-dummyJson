...
const ProductDetail = ({ onAddToCart = () => {} }) => {
    ...
    const handleAddToCart = () => {
        ...
        onAddToCart(`ID: ${product.id} | Name: ${product.title} | Price: ${product.price} | Quantity: ${quantity}`);
    ...
    return(
        <div className='relative'>
            <div className='m-auto flex flex-col items-center justify-center p-16 bg-gray-200'>
                <div className="flex flex-col gap-6">
                    ...
                    <div className="right flex justify-between gap-16 border-gray-400 border-t px-4 py-8">
                        <div className='space-y-2 max-w-[50vw]'>
                            <div className='flex flex-wrap justify-between'>
                                <h1 className='text-4xl'>{product.title}</h1>
                                <div className="flex gap-2 items-center border-2 border-lime-400 px-2 py-2 rounded-4xl">
                                    <input
                                        type="number"
                                        min="1"
                                        value={quantity}
                                        ...
                                        className="w-40 px-2 py-1 border-lime-400 text-center outline-none"
                                        />
                                    <button onClick={handleAddToCart} className='flex justify-center items-center text-center gap-4 rounded-4xl bg-lime-400 px-4 py-2 cursor-pointer group transition duration-300 hover:scale-110 hover:drop-shadow-xl hover:drop-shadow-gray-600/20'>
                                ...
                                <div key={index} className='flex flex-col gap-2 my-4 p-2 bg-gray-100 rounded'>
                                    <div className='flex justify-between bg-gray-300 p-2 rounded'>
                                        <div className='flex gap-4 justify-between items-center text-center'>
                                            <h1 className='text-xl text-black'>{review.reviewerName}</h1>
                                            <h2 className='text-sm text-gray-400'>Email: {review.reviewerEmail}</h2>
                                        </div>
                                    ...
                                    <h2 className='p-2'>{review.comment}</h2>
                                </div>
                            ))}
                        </div>
                    </div>
